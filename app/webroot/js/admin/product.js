...
$(document).on('click', '.edit-product', function() {
    ...
    $('#editor2')[0].value = $(this).parent('.btn-product').prev('.info-product').children('.des-product').children('div').children('strong').html();
});
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result == 'duplicatedC') {
                $('#addCategory').find('.notBlank').children('label').text('Category already exists');
